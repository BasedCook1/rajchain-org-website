...
export async function RajChainEventsImport() {
  ...
  console.log("Importing RajChain Events from Google Sheet")
  const currentYear = new Date().getFullYear()
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${currentYear}%20RajChain%20Events!B:H?majorDimension=COLUMNS&key=${googleApiKey}`
  )
